...
compare.LIPR.plots.refine <- function(my_design, results_dir, variable, var_levels = c("default"), var_labels=c("default"), 
  ...
  if(plot_mode == "proportion"){
    ...
    t_overlap <- as.data.frame(svymean(as.formula(paste("~", variable)), design = subset(my_design, shared_JAM ==1 & grpd_tenure == 1),
                                            na.rm=TRUE))
    setDT(t_overlap, keep.rownames = TRUE)[]
    colnames(t_overlap)[1] <- variable
    t_overlap$definition <- "Overlap group PRS"
    ...
    table_list <- list(t_noshared, t_overlap, t_JAMnohbrf_greq, t_allworkage)
    t_prop<- rbindlist(table_list, fill = TRUE) 
    rm(table_list)
    rm(t_noshared, t_overlap, t_JAMnohbrf_greq, t_allworkage)
