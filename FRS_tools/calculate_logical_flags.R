calculate.logical.flags <- function(data_table, sv_year){
  ...
  #Flag for has lodger
  data_table[ is.na(CONVBL), CONVBL := 999]
  data_table[ , has_lodger := ifelse( (sum(CONVBL == 1 | CONVBL == 2) >0) , 1, 0 ), by=.(SERNUM, year)]
  
  #Employment of whole household
  data_table[ , workless_hhld := FALSE]
  data_table[ , workless_hhld := ifelse( (sum(ECOBU == 6 | ECOBU == 7 | ECOBU == 8) >0) , TRUE, FALSE ), by=.(SERNUM, year)]
  #HRP unemployed or inactive
  data_table[ , hrp_nowork := FALSE]
  data_table[ , hrp_nowork := ifelse( is_HRP == 1 & (DVIL04A == 3 | DVIL04A == 4), TRUE, FALSE)]
